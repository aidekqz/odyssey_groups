...
# docker exec -it postgres bash -c "sed -i '1 i\host postgres postgres all trust' /var/lib/postgresql/data/pg_hba.conf"
...
sleep 2
psql "host=127.0.0.1 port=5432 user=postgres password=postgres123 dbname=postgres" -f init.sql
pkill odyssey || true
# ./odyssey ./config.conf
